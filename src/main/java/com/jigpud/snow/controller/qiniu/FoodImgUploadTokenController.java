...
@Slf4j
@RestController
public class FoodImgUploadTokenController extends BaseController {
    private final QiniuService qiniuService;

    @Autowired
    FoodImgUploadTokenController(QiniuService qiniuService) {
        this.qiniuService = qiniuService;
    }

    @GetMapping(PathConstant.GET_FOOD_IMG_UPLOAD_TOKEN)
    ...
    ResponseBody<UploadTokenResponse> getFoodImgUploadToken() {
        String filename = "food/" + Encryptor.uuid();
        String uploadToken = qiniuService.createImgUploadToken(filename);
        log.debug("foodImageUploadToken -> uploadToken: {}, filename: {}", uploadToken, filename);
